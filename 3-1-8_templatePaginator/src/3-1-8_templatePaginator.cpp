...
template <typename Iterator>
auto container_slice(Iterator& first, Iterator& last, size_t top) {
	return IteratorRange(first, next(first, min<size_t>(top, last-first)));
}
	...
	:first(begin),
	 ...
	 page_size(page_size){
		...
		for (; begin < end; begin+=page_size) {
			_pages.push_back(container_slice(begin, end, page_size));
		}
	}
